...
![График масштаба проектов](outputs/project_scale_by_language.png)<br /><br />
Анализ 4.1 миллиона проектов GitLab показал, что масштаб проектов сильно зависит от специализации языка. Из-за ограничений API прямые данные о размерах репозиториев недоступны, поэтому масштаб оценивался по косвенным метрикам: количеству звёзд (популярность), форков (использование) и issues (сложность).<br /><br />
Наибольший масштаб демонстрируют специализированные языки: **Curry** для функционального программирования, **Rouge** для встраиваемых систем и **Wren** для игровых движков. Эти языки характерны для сложных проектов с высокой вовлеченностью сообщества.<br /><br />
В категории крупных проектов преобладают современные инструментальные языки: **Janet** (Lisp-диалект), **jq** (обработка JSON) и **P4** (программируемые сети). Средний масштаб характерен для языков разметки и конфигурации - **WebIDL** и **TOML**.<br /><br />
Наибольшее количество проектов (15,916) использует **MATLAB** для научных вычислений, однако они относятся к минимальным по масштабу, что указывает на преобладание небольших исследовательских скриптов над комплексными системами. Аналогичная ситуация наблюдается с **OpenSCAD** (3D-моделирование) и **Markdown** (документация).<br /><br />
Распределение показывает, что популярность языка не всегда коррелирует с масштабом проектов - нишевые языки часто используются в более сложных и специализированных системах, в то время как широко распространенные языки могут применяться в основном для небольших утилит и скриптов. 
