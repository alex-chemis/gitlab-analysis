...
def bar_chart(labels: Sequence[str], values: Sequence[float], out_path: str | Path,
              title: str = "", xlabel: str = "", ylabel: str = "", rotate_x: int = 45) -> Path:
...
def barh_chart(labels: Sequence[str], values: Sequence[float], out_path: str | Path,
...
def pie_chart(labels: Sequence[str], values: Sequence[float], out_path: str | Path,
              title: str = "", random_colors: bool = False, seed: Optional[int] = None) -> Path:
    out_p = ensure_parent_dir(out_path)
    ...
    wedges, texts, autotexts = plt.pie(
        values,
        autopct=lambda p: _fmt(p, values),
        startangle=90,
        colors=colors
    )
    plt.title(title)
